...
val df = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").load("datasets/b5dataset.csv")
var dfRdd = df.select( "dog_SubStatusCode","StoolFirm","EnergyLevel","RespondsToCommandKennel","GoodWStrangers","BehavesWellClass","RaidsGarbage","GoodAppetite","FoodType","ExerciseType","Sex","Breed","DayInLife").rdd.map{
...
val hashingTF3 = new HashingTF().setInputCol("filtered6").setOutputCol("rawfeatures6").setNumFeatures(20)
