...
val testingRowRDD = sc.makeRDD(rowRDD.take((rowRDD.count * 0.8).toInt))
...
val indexer2 = new StringIndexer().setInputCol("text2").setOutputCol("text2Index").setHandleInvalid("skip")
...
val hashingTF1 = new HashingTF().setInputCol("filtered3").setOutputCol("rawfeatures3").setNumFeatures(1024)
...
val lr = new LogisticRegression().setMaxIter(200).setRegParam(1.1)
