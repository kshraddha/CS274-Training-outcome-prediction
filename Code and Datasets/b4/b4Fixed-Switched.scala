...
val df = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").load("b4/datasets/b4dataset.csv")
var rowRDD = df.select( "dog_SubStatusCode","Health","EnergyLevel","RespondsToCommandKennel","GoodWStrangers","BehavesWellClass","NailCutting","GoodAppetite","FoodType","ExerciseType","Sex","Breed").rdd
