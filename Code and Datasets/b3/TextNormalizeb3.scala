...
val df = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").load("datasets/b3dataset.csv")
var dfRdd = df.select( "dog_SubStatusCode","GoodAppetite","FoodType","ExerciseType","dog_Sex","dbc_DogBreedDescription").rdd.map{
case Row( label:String,text1:String, text2:String, text3:String, text4:String, text5:String) => (label, text1, textNormalize(text2), textNormalize(text3), text4, text5)
}.toDF("label","text1","text2","text3","text4","text5")
...
val vectorA = new VectorAssembler().setInputCols(Array("text1Index", "text2Index", "text3Index", "features4","features5")).setOutputCol("features")
